<template>
  <div class="flex flex-col items-center">
    <label>
      <div>U<sub>h</sub> = {{ coilVoltage.toFixed(2) }}V</div>
      <input
        v-model.number="coilVoltage"
        min="0.0"
        max="10.0"
        type="range"
        step="any"
    ...
    <label>
      <div>U<sub>a</sub> = {{ screenVoltage.toFixed(2) }}KV</div>
      <input
        v-model.number="screenVoltage"
        min="0.0"
        max="10.0"
        type="range"
        step="any"
    ...
    >
      ...
      <path
        ...
        fill="none"
        stroke="#F60"
        :stroke-width="0.7 + coilDependence"
...
<style scoped>
.coil {
  filter: drop-shadow(0 0 v-bind(coilDependence) rgb(255, 187, 0));
}

.field-lines {
  & > path {
    stroke-width: v-bind(0.6 + beamIntensity);
